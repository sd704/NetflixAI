        ...
        <div className="flex justify-center w-full mt-8">
            {filteredResults?.length > 0 && <div id="grid" className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 px-4 max-w-7xl ">
                {filteredResults?.map((item) => <div
                    ...
                    className="flex-shrink-0 h-64 md:h-80 m-2 md:my-8 md:mx-6 aspect-[2/3] bg-center bg-cover rounded-lg cursor-pointer active:h-60 md:hover:h-96 md:hover:my-0 md:hover:mx-0 md:hover:shadow-lg transition-all"
            ...
            {isLoading &&
                <div id="grid" className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 px-4 max-w-7xl ">
                    {[...Array(5)].map((_, i) => (< div
                        key={i}
                        className="flex-shrink-0 h-64 md:h-80 m-2 md:my-8 md:mx-6 aspect-[2/3] shimmer-background rounded-lg cursor-pointer transition-all">
                    </div>))}
                </div>
            }
